...
import com.uisftech.cloan.limit.config.metrics.SpectatorLogMetricWriter;
...
public class MetricsConfiguration extends MetricsConfigurerAdapter {
    ...
    public void init() {
        ...
        if (scfsProductProperties.getMetrics().getJmx().isEnabled()) {
            log.debug("Initializing Metrics JMX reporting");
            JmxReporter jmxReporter = JmxReporter.forRegistry(metricRegistry).build();
            jmxReporter.start();
        }

        if (scfsProductProperties.getMetrics().getLogs().isEnabled()) {
            ...
            final Slf4jReporter reporter = Slf4jReporter.forRegistry(metricRegistry)
                ...
                .build();
            reporter.start(scfsProductProperties.getMetrics().getLogs().getReportFrequency(), TimeUnit.SECONDS);
        }
    }
    ...
    public static class GraphiteRegistry {
        ...
        @PostConstruct
        private void init() {
            if (scfsProductProperties.getMetrics().getGraphite().isEnabled()) {
                log.info("Initializing Metrics Graphite reporting");
                String graphiteHost = scfsProductProperties.getMetrics().getGraphite().getHost();
                Integer graphitePort = scfsProductProperties.getMetrics().getGraphite().getPort();
                String graphitePrefix = scfsProductProperties.getMetrics().getGraphite().getPrefix();
    ...
    public static class SparkRegistry {
        ...
        @PostConstruct
        private void init() {
            if (scfsProductProperties.getMetrics().getSpark().isEnabled()) {
                log.info("Initializing Metrics Spark reporting");
                String sparkHost = scfsProductProperties.getMetrics().getSpark().getHost();
                Integer sparkPort = scfsProductProperties.getMetrics().getSpark().getPort();
    ...
    @Bean
    @ConditionalOnProperty("scfsloanms.logging.spectator-metrics.enabled")
    @ExportMetricReader
