...
public class MetricsConfiguration extends MetricsConfigurerAdapter {
    ...
    public void init() {
        ...
        if (scfsLoanProperties.getMetrics().getJmx().isEnabled()) {
            log.debug("Initializing Metrics JMX reporting");
            JmxReporter jmxReporter = JmxReporter.forRegistry(metricRegistry).build();
            jmxReporter.start();
        }

        if (scfsLoanProperties.getMetrics().getLogs().isEnabled()) {
            ...
            final Slf4jReporter reporter = Slf4jReporter.forRegistry(metricRegistry)
                ...
                .build();
            reporter.start(scfsLoanProperties.getMetrics().getLogs().getReportFrequency(), TimeUnit.SECONDS);
        }
    }
    ...
    public static class GraphiteRegistry {
        ...
        @PostConstruct
        private void init() {
            if (scfsLoanProperties.getMetrics().getGraphite().isEnabled()) {
                log.info("Initializing Metrics Graphite reporting");
                String graphiteHost = scfsLoanProperties.getMetrics().getGraphite().getHost();
                Integer graphitePort = scfsLoanProperties.getMetrics().getGraphite().getPort();
                String graphitePrefix = scfsLoanProperties.getMetrics().getGraphite().getPrefix();
    ...
    public static class SparkRegistry {
        ...
        @Inject
        private CloanLimitProperties demandProperties;

        @PostConstruct
        private void init() {
            if (demandProperties.getMetrics().getSpark().isEnabled()) {
                log.info("Initializing Metrics Spark reporting");
                String sparkHost = demandProperties.getMetrics().getSpark().getHost();
                Integer sparkPort = demandProperties.getMetrics().getSpark().getPort();
